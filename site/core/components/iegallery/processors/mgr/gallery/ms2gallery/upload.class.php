<?php
include_once MODX_CORE_PATH . 'components/ms2gallery/processors/mgr/gallery/upload.class.php';

class IeGalleryMs2GalleryUploadProcessor extends msResourceFileUploadProcessor
{
    ...
    public function initialize()
    {
        ...
        $source = isset($properties['media_source']) ? $properties['media_source'] : $this->modx->getOption('ms2gallery_source_default');
    ...
    public function process()
    {
        ...
        if ($this->modx->getOption('ms2gallery_duplicate_check', null, true, true)) {
            if ($tmpObject = $this->modx->getObject('msResourceFile',
                array('resource_id' => $this->resource->id, 'hash' => $data['hash'], 'parent' => 0))
            ) {
                @unlink($data['tmp_name']);
                return $this->failure($this->modx->lexicon('ms2gallery_err_gallery_exists'), array('code' => 304, $tmpObject->get('id'), 'file' => $tmpObject->get('file'), 'name' => $tmpObject->get('name')));
            }
        }
        ...
        $filename = !empty($properties['imageNameType']) && $properties['imageNameType'] == 'friendly'
            ? $this->resource->cleanAlias($filename, array(
                ...
                'friendly_alias_restrict_chars_pattern' => '#[^\w\s.-]#iu',
     ...
     * @return bool|void
     */
    public function resize($file, $opt = array())
    {
        if (!$this->getProperty('resize_upload_image', false)) return;
        $tmp = getimagesize($file);
        if (is_array($tmp)) {
            $width = $tmp[0];
            $height = $tmp[1];
        } else if (strpos(basename($file), '.webp') !== false) {
        ...
        $properties = $this->mediaSource->getPropertyList();
        if ($width > $properties['maxUploadWidth'] || $height > $properties['maxUploadHeight']) {
            $options = array();
            if ($properties['maxUploadWidth'] > $properties['maxUploadHeight']) {
                $options['h'] = $properties['maxUploadHeight'];
            } else {
                $options['w'] = $properties['maxUploadWidth'];
            }
            $options = array_merge(array(
                'q' => $properties['thumbnailQuality'],
                'far' => '1',
                'bg' => 'ffffff'
            ), $options, $opt);
            ...
            if ($phpThumb->GenerateThumbnail()) {
                if (!$phpThumb->RenderToFile($file)) {
                    $this->modx->log(modX::LOG_LEVEL_ERROR, '[phpThumbOf] Could not cache thumb "' . $file . '"  - Debug: ' . print_r($phpThumb->debugmessages, true));
                }
            } else {
                $this->modx->log(modX::LOG_LEVEL_ERROR, '[resize] Could not generate thumbnail: ' . $file . ' - Debug: ' . print_r($phpThumb->debugmessages, true));
            }
        }
        return true;
...
return 'IeGalleryMs2GalleryUploadProcessor';
