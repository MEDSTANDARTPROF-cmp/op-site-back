...
class mSearch2 {
	...
	public function getWorkFields($config = array()) {
        ...
        $response = $this->invokeEvent('mse2OnGetWorkFields', array('fields' => $fields));
        if ($response['success']) {
	...
	public function getFilters($ids, $build = true) {
		...
		if ($build && $cache = $this->modx->cacheManager->get($this->config['cache_prefix'] . 'prep_' . md5(implode(',', $ids) . $this->config['filters']))) {
		...
		elseif (!$build && $this->filters = $this->modx->cacheManager->get($this->config['cache_prefix'] . 'fltr_' . md5(implode(',', $ids)))) {
			return $this->filters;
		}
		...
		if (!empty($this->config['cacheTime'])) {
			$this->modx->cacheManager->set($this->config['cache_prefix'] . 'prep_' . md5(implode(',', $ids) . $this->config['filters']), $cache, $this->config['cacheTime']);
		}
	...
	public function getSuggestions($ids, array $request, array $current = array()) {
        ...
        $possible_filters = array_flip(array_merge(array_keys($built), array_values($this->aliases)));
        foreach ($request as $k => $v) {
