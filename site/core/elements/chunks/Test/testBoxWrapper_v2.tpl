...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('[id^="test-question-"]').each(function () {
            ...
            if (filter.length > 0 && !filter.includes(parseInt(key - 1))) {
                console.log('skip', key, filter);
                return;
            }
        ...
        // Выводим массив вопросов в консоль
        ...
        jQuery(document).on('click', '#test-repeat-answer', () => {
            ...
            $('[id^="test-answer-' + questions[currentQuestion].key + '-"]').each(function () {
                ...
                if ($(this).is(':checked')) {
                    ...
                    if (!questions[currentQuestion].correct_answer.includes(key)) {
        ...
        jQuery(document).on('click', '#test-confirm-answer', () => {
            ...
            answerForCurrentQuestionIsTrue = compareArrays(checkedInputs, questions[currentQuestion].correct_answer);
            completeQuestions[currentQuestion] = {
                answer: checkedInputs,
            };
            updateBall();
            ...
            jQuery(document).find('#test-ball').text(ball);
            jQuery(document).find('#test-wrong-ball').text(wrongBall);
        ...
        let updateProgress = () => {
            ...
            completeQuestions.map((completeQuestion, index) => {
                answerForCurrentQuestionIsTrue = compareArrays(completeQuestion.answer, questions[index].correct_answer);
                debugger;
                ...
                //if (questions[index].correct_answer.includes(completeQuestion.answer)) {
                //    succesProgressProcent++;
                //} else {
                //    wrongProgressProcent++;
                //}
        ...
        let compareArrays = (arr1, arr2) => {
            // Если длина массивов не одинаковая, они не могут совпадать
            if (arr1.length !== arr2.length) {
                return false;
            }
        
            // Сортируем массивы и сравниваем их элементы
            arr1.sort();
            arr2.sort();
        
            // Проверяем, совпадают ли элементы на каждой позиции
